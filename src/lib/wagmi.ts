...
export const config = createConfig({
  ...
  connectors: [
    walletConnect({ 
      projectId,
      metadata: {
        name: 'INTENT Campaign Engine',
        description: 'On-chain marketing & activity execution engine for Arc Network',
        url: typeof window !== 'undefined' ? window.location.origin : 'https://intent.app',
    ...
    coinbaseWallet({
      appName: 'INTENT Campaign Engine',
    }),
  ],
  transports: {
...
// Initialize Web3Modal synchronously
